SHAPE_MODE = "CSV"
CSV_FILE = "shapes/bunny2D.csv"
SHAPE_SCALE = 1.0  # Make it bigger/smaller
SHAPE_CENTER = (0.7, 0.4)  # Move it around

def draw_shape_on_axis(ax, polygon, facecolor='gray', edgecolor='black', linewidth=2, alpha=0.9, zorder=10):
    """Draw arbitrary shape (polygon) on matplotlib axis"""
    try:
        if hasattr(polygon, 'exterior'):
            # Single polygon
            coords = list(polygon.exterior.coords)
            patch = MPLPolygon(coords, facecolor=facecolor, edgecolor=edgecolor,
                               linewidth=linewidth, alpha=alpha, zorder=zorder)
            ax.add_patch(patch)
        elif hasattr(polygon, 'geoms'):
            # Multi-polygon (collection of shapes)
            ...
            # Fallback: try to treat as point and create circle
            if hasattr(polygon, 'x') and hasattr(polygon, 'y'):
                circle = Circle((polygon.x, polygon.y), 0.08, facecolor=facecolor,
                                edgecolor=edgecolor, linewidth=linewidth, alpha=alpha, zorder=zorder)
                ax.add_patch(circle)
    except Exception as e:
        print(f"Error drawing shape: {e}")
        # Fallback to circle at center
        circle = Circle(SHAPE_CENTER, 0.08, facecolor=facecolor,
                        edgecolor=edgecolor, linewidth=linewidth, alpha=alpha, zorder=zorder)
        ax.add_patch(circle)
        """
Fixed Wind Tunnel with Proper Moving Particles
...
def advect_particles_explicit(seed_positions, fluid_state, dt):
    """Explicit particle advection with VISIBLE movement - works with arbitrary shapes"""
    new_positions = seed_positions.copy()
    movements = np.zeros(len(seed_positions))

    # FORCE LARGE MOVEMENTS for visibility
    ...
    for i in range(len(seed_positions)):
        if i < len(fluid_state):
            ...
            # Check if particle would enter solid (works for any shape)
            new_point = Point(new_pos)
            safety_distance = 0.01  # Minimum distance from solid boundary

            if solid_polygon.distance(new_point) > safety_distance:
                new_positions[i] = new_pos
            else:
                # Particle too close to or inside solid - push it away
                try:
                    # Find closest point on solid boundary
                    boundary = solid_polygon.boundary
                    closest_point = boundary.interpolate(boundary.project(new_point))
                    closest_coords = np.array([closest_point.x, closest_point.y])

                    # Calculate direction away from solid
                    direction = old_pos - closest_coords
                    distance = np.linalg.norm(direction)

                    if distance > 1e-8:
                        direction = direction / distance
                        # Push particle away from solid
                        new_positions[i] = closest_coords + direction * safety_distance * 2
                    else:
                        # Fallback: small random displacement
                        new_positions[i] = old_pos + np.random.normal(0, 0.01, 2)
...
def animate_wind_tunnel():
    ...
    gamma = 1.4
    inflow_rho = 1.0
    inflow_u = 0.4  # Increased for more visible movement
    inflow_v = 0.0
    inflow_p = 1.0
    ...
    def update(frame):
        ...
        try:
            ...
            if frame % 20 == 0:
                ...
                print(
                    f"Frame {frame}: {len(seed_positions)} particles, avg movement: {avg_movement:.4f}, max: {max_movement:.4f}")
        ...
        try:
            ...
            # Draw solid shape (works for any geometry)
            draw_shape_on_axis(ax1, solid_polygon, facecolor='orange',
                               edgecolor='black', linewidth=2, alpha=0.8, zorder=10)

            ax1.set_xlim(0, domain_size[0])
            ax1.set_ylim(0, domain_size[1])
            ax1.set_aspect('equal')
            shape_name = CSV_FILE.split('/')[-1].replace('.csv', '') if SHAPE_MODE == "csv" else "default"
            ax1.set_title(f'Pressure & Particles (Frame {frame}, N={len(seed_positions)}) - {shape_name.title()}')
            ...
            if len(geometry) > 0:
                ...
                # Draw solid shape
                draw_shape_on_axis(ax2, solid_polygon, facecolor='gray',
...
if __name__ == "__main__":
    print("🌪️ Arbitrary Shape Wind Tunnel Simulation")
    print("=" * 50)

    print(f"Current configuration:")
    print(f"- Shape mode: {SHAPE_MODE}")
    print(f"- CSV file: {CSV_FILE}")
    print(f"- Shape scale: {SHAPE_SCALE}")
    print(f"- Shape center: {SHAPE_CENTER}")
    print()

    print("Available sample shapes:")
    for shape_file in ['triangle.csv', 'square.csv', 'bunny2D.csv', 'airfoil.csv']:
        if os.path.exists(f'shapes/{shape_file}'):
            print(f"  ✅ shapes/{shape_file}")
        else:
            print(f"  ❌ shapes/{shape_file}")
    print()

    print("💡 To use your own shape:")
    print("1. Create a CSV file with 'x' and 'y' columns")
    print("2. Points should define the boundary of your shape in order")
    print("3. Update CSV_FILE variable to point to your file")
    print("4. Adjust SHAPE_SCALE and SHAPE_CENTER as needed")
    print()
